...
image gray_scale(image in)
{
	...
	for (row = 0; row < in->h; row ++) {
		for (col = 0; col < in->w; col++) {	
			out->pix[row][col].r = (in->pix[row][col].r + in->pix[row][col].g + in->pix[row][col].b)/3;
			out->pix[row][col].g = (in->pix[row][col].r + in->pix[row][col].g + in->pix[row][col].b)/3;
			out->pix[row][col].b = (in->pix[row][col].r + in->pix[row][col].g + in->pix[row][col].b)/3;
		}
	}
