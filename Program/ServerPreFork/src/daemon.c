...
#define CONFIG_FILE_DEFAULT_PORT "PORT=8007"
...
#define CONFIG_FILE_DEFAULT_FORKS "FORKS=5"
...
void createConfigFile()
{
    ...
    else
    {
        ...
        fputs("\n\n", file);
        fputs(CONFIG_FILE_DEFAULT_FORKS, file);
        fclose(file);
    }
...
/*
 *  Returns the specified processes amount, returns a default of 5 if it's not found.
 */
int *getForkFromConfigFile()
{
    ...
    int *forks = calloc(1, sizeof(int));

    while (!feof(file))
    {
        if (fscanf(file, "FORKS=%d", forks) == 1)
        {
            break;
        }
        fgetc(file);
    ...
    {
        perror("Forks not found on config file\n");
        *forks = 5;
        return forks;
    }
...
int main(int argc, char **argv, char **envp)
{
    ...
    int forksAmount = *getForkFromConfigFile();
    printf("Amount of forks: %d\n", forksAmount);
    pid_t pid;
    pid_t pidArray[forksAmount];
    int currentChild = 0;
    for (int i = 0; i < forksAmount; i++)
    {
        ...
        {
            stamp();
            fprintf(log_stream, "%s:New child process: #%d\n", timestamp, getpid());
            printf("New Child: #%d\n", getpid());
            break;
        }
        pidArray[i] = pid;
    }
    //stop every child process to wait for a connection
    if (pid == 0)
    {
        stamp();
        fprintf(log_stream, "%s:Stopping process: #%d\n", timestamp, getpid());
        kill(getpid(), SIGSTOP);
        stamp();
        fprintf(log_stream, "%s:Continuing process: #%d\n", timestamp, getpid());
    }
    ...
    while (running == 1)
    {
        ...
        if (pid != 0)
        {
            if (currentChild == forksAmount)
            {
                currentChild = 0;
            }
            stamp();
            fprintf(log_stream, "%s:Reviving child on parent: %d \n", timestamp, pidArray[currentChild]);
            kill(pidArray[currentChild], SIGCONT);
            currentChild += 1;
            stamp();
        ...
        else
        {
            ...
            close(nextSocket);
            exiting = 1;
            //stops the child process after it finishes
            stamp();
            fprintf(log_stream, "%s:Stopping process: #%d\n", timestamp, getpid());
            kill(getpid(), SIGSTOP);
            stamp();
